    ...
    <script>
        ...
        document.querySelectorAll('.chart-period .period-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                loadUserGrowth(period); // or loadUserTypes(period)
            });
        });

        const ctx1 = document.getElementById('userTypesChart').getContext('2d');

        const userTypesChart = new Chart(ctx1, {
            type: 'pie',
            data: {
                labels: ['Admin', 'Company', 'Seekers'],
                datasets: [{
                    label: 'User Types',
                    data: [10, 25, 65],
                    backgroundColor: [
